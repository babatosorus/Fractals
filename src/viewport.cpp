...
std::pair<double,double> Viewport::screenCoordToComplex(const int&x, const int& y)
{
	double realFactor = (m_MaxReal - m_MinReal)/(m_Width -1);  
	double imaginaryFactor = (m_MaxImaginary - m_MinImaginary)/(m_Height -1);

	return std::make_pair(m_MinReal + x*realFactor, m_MinImaginary + y*imaginaryFactor);
}
